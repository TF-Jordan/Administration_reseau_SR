...
def vectorize_livreurs(
    embedding_service: EmbeddingService,
    vector_store: VectorStore,
    batch_size: int = 50,
) -> int:
    """
    Vectorize all livreurs.
    ...
    Returns:
        Number of livreurs vectorized
    """
    logger.info("Starting livreur vectorization...")

    session = get_sync_session()
    try:
        # Get all livreurs
        livreurs = livreur_repository.get_all_sync(session)
        logger.info(f"Found {len(livreurs)} livreurs to vectorize")

        if not livreurs:
            logger.warning("No livreurs found in database")
            return 0

        # Create collection
        vector_store.create_collection_sync(ProductType.LIVREUR, recreate=True)

        # Prepare items for vectorization
        items = []
        for livreur in livreurs:
            description = livreur.to_description()
            vector = embedding_service.encode_for_qdrant(description)

            items.append({
                "real_product_id": str(livreur.id),
                "vector": vector,
                "metadata": {
                    "nom_commerciale": livreur.nom_commerciale,
                    "statut": livreur.statut,
                    "disponible": livreur.disponible,
                    "localisation": livreur.localisation,
                    "reputation": livreur.reputation,
        ...
        for i in range(0, len(items), batch_size):
            batch = items[i:i + batch_size]
            count = vector_store.upsert_vectors_batch(ProductType.LIVREUR, batch)
            total += count
            logger.info(f"Vectorized batch {i // batch_size + 1}: {count} livreurs")

        logger.info(f"Livreur vectorization complete: {total} vectors created")
        return total
...
def main():
    ...
    if args.type in ["livreurs", "all"]:
        results["livreurs"] = vectorize_livreurs(
            embedding_service, vector_store, args.batch_size
        )
