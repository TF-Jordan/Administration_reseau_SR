...
class Livreur(Base):
    """
    Delivery person (Livreur) model.
    Used for the delivery platform recommendation path.
    """
    ...
    registre_commerce: Mapped[Optional[str]] = mapped_column(String(255))
    nom_commerciale: Mapped[Optional[str]] = mapped_column(String(255))
    nui: Mapped[Optional[str]] = mapped_column(String(100), unique=True)

    # Additional useful attributes
    disponible: Mapped[bool] = mapped_column(Boolean, default=True, index=True)
    nombre_livraisons: Mapped[int] = mapped_column(Integer, default=0)
    note_moyenne: Mapped[float] = mapped_column(Float, default=0.0)
    zone_couverture: Mapped[Optional[str]] = mapped_column(Text)
    ...
    # Foreign key to Personne (owner/associated person)
    personne_id: Mapped[Optional[uuid.UUID]] = mapped_column(
        UUID(as_uuid=True),
        ForeignKey("personnes.id"),
        nullable=True,
    )

    def __repr__(self) -> str:
        return f"<Livreur(id={self.id}, statut={self.statut}, reputation={self.reputation})>"
    ...
    def to_description(self) -> str:
        ...
        if self.nom_commerciale:
            parts.append(f"Livreur {self.nom_commerciale}")
        else:
            parts.append("Livreur professionnel")

        parts.append(f"avec une réputation de {self.reputation:.1f}/5")

        if self.localisation:
            parts.append(f"basé à {self.localisation}")

        if self.zone_couverture:
            parts.append(f"couvrant la zone {self.zone_couverture}")

        parts.append(
            f"ayant effectué {self.nombre_livraisons} livraisons"
        )

        if self.disponible:
            parts.append("actuellement disponible")
        else:
